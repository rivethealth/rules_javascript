...
var sub = function sub(pattern, ...values) {
  ...
  let result = pattern.replace(regex, function (
    _,
    is_literal,
    is_left_align,
    is_padded,
...
class TextWrapper {
  ...
  _handle_long_word(reversed_chunks, cur_line, cur_len, width) {
    ...
    if (this.break_long_words) {
      ...
      reversed_chunks[reversed_chunks.length - 1] = reversed_chunks[
        reversed_chunks.length - 1
      ].slice(space_left);
...
var argparse = createCommonjsModule(function (module) {
  ...
  function _alias(object, from, to) {
    try {
      ...
      Object.defineProperty(object, from, {
        value: util__default["default"].deprecate(
          object[to],
  ...
  function deprecate(id, string) {
    _deprecations[id] =
      _deprecations[id] || util__default["default"].deprecate(() => {}, string);
    _deprecations[id]();
  }
        ...
        _format_actions_usage(actions, groups) {
          ...
          for (let group of groups) {
            ...
            if (start === -1) {
              ...
              if (
                _array_equal(actions.slice(start, end), group._group_actions)
              ) {
          ...
          for (let [i, action] of Object.entries(actions)) {
            ...
            if (action.help === SUPPRESS) {
              ...
              let default_value = this._get_default_metavar_for_positional(
                action,
              );
              ...
              if (action.nargs === 0) {
                ...
                let default_value = this._get_default_metavar_for_optional(
                  action,
                );
    ...
    class _StoreAction extends Action {
      constructor() {
        ...
        if (const_value !== undefined && nargs !== OPTIONAL) {
          throw new TypeError(
            sub("nargs must be %r to supply const", OPTIONAL),
          );
        }
        super({
          option_strings,
  ...
  const _HelpAction = _callable(
    class _HelpAction extends Action {
      constructor() {
        let [option_strings, dest, default_value, help] = _parse_opts(
          arguments,
          {
            option_strings: no_default,
            dest: SUPPRESS,
  ...
  const FileType = _callable(
    class FileType extends Function {
      ...
      call(string) {
        ...
        let fd;
        try {
          fd = fs__default["default"].openSync(
            string,
            this._flags,
            this._options.mode,
          );
          ...
          ["storeConst", "storeTrue", "storeFalse", "appendConst"].forEach(
            (old_name) => {
              ...
              this.register(
                "action",
                old_name,
                util__default["default"].deprecate(
      ...
      class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
        ...
        constructor() {
          ...
          this.register("type", "int", function (x) {
            ...
            if (!Number.isInteger(result)) {
              throw new TypeError(
                sub("could not convert string to int: %r", x),
              );
            }
            return result;
          });
          ...
          let default_prefix = prefix_chars.includes("-")
            ? "-"
            : prefix_chars[0];
          if (this.add_help) {
            this.add_argument(
        ...
        _get_positional_actions() {
          return this._actions.filter(
            (action) => !action.option_strings.length,
          );
        }
        ...
        _read_args_from_files(arg_strings) {
          ...
          for (let arg_string of arg_strings) {
            ...
            if (
              ...
              try {
                let args_file = fs__default["default"].readFileSync(
                  arg_string.slice(1),
                  "utf8",
                );
                let arg_strings = [];
        ...
        _match_argument(action, arg_strings_pattern) {
          ...
          let match = arg_strings_pattern.match(
        ...
        _parse_optional(arg_string) {
          ...
          if (arg_string.includes("=")) {
            let [option_string, explicit_arg] = _string_split(
              arg_string,
              "=",
              1,
            );
...
`;
  fs__namespace.writeFileSync(args.outputPath, content, "utf8");
}

function rootArg(value) {
  return Root.json().fromJson(JSON.parse(value));
}
function extraDepArg(value) {
