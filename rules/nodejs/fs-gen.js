...
function addDep(root, name, path) {
    ...
    dir.children.set(parts[parts.length - 1], { type: VfsEntry.LINK, path: [path] });
}
function gen(args) {
    const paths = new Map();
    ...
    for (const root_ of args.roots) {
        ...
        const t = new Set();
        packageRoot.children.set('package.json', { type: VfsEntry.PATH, path: root_.descriptor });
        packageRoot.children.set('node_modules', { type: VfsEntry.DIRECTORY, children: new Map() });
    ...
    for (const entry of args.entries) {
        ...
        dir.children.set(parts[parts.length - 1], { type: VfsEntry.PATH, path: entry.file });
    }
...
const parser = new argparse.ArgumentParser({ description: 'Resolve dependency information into LinkFs' });
