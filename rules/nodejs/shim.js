...
var ts = createCommonjsModule(function (module, exports) {
...
class Resolver {
    constructor(trace, variations) {
        ...
        tslib.__classPrivateFieldSet(this, _trace, trace);
        tslib.__classPrivateFieldSet(this, _variations, variations);
    }
    /**
     ...
     */
    addGlobal(id) {
        tslib.__classPrivateFieldGet(this, _globals).add(id);
    }
    /**
     * Add package
     */
    ...
    resolveById(id, request) {
        const package_ = tslib.__classPrivateFieldGet(this, _packageById).get(id);
        if (!package_) {
    ...
    resolve(request, parent) {
        const packageModule = tslib.__classPrivateFieldGet(this, _pathToModule).get(parent);
        if (!packageModule) {
    ...
    _resolveDeps(deps, request, attempts) {
        for (const dep of deps) {
            const package_ = tslib.__classPrivateFieldGet(this, _packageById).get(dep.id);
            if (!package_) {
        ...
        for (const id of tslib.__classPrivateFieldGet(this, _globals)) {
            const package_ = tslib.__classPrivateFieldGet(this, _packageById).get(id);
            if (!package_) {
    ...
    _resolveDep(request, id, package_, name, attempts) {
        ...
        attempts.push(id);
        for (const name of tslib.__classPrivateFieldGet(this, _variations).call(this, requestPart)) {
    ...
    static readManifest(resolver, path, runfile) {
        const items = fs__default['default']
...
var ts$1 = createCommonjsModule(function (module, exports) {
var _pathByName;
Object.defineProperty(exports, "__esModule", { value: true });
...
class Runfiles {
    constructor() {
        _pathByName.set(this, new Map());
    }
    addRunfile(name, path) {
        tslib.__classPrivateFieldGet(this, _pathByName).set(name, path);
    }
    getPath(name) {
        if (name.startsWith("../")) {
        ...
        if (RUNFILES_MANIFEST) {
            const path = tslib.__classPrivateFieldGet(this, _pathByName).get(name);
            if (!path) {
...
_pathByName = new WeakMap();
const runfiles = new Runfiles();
if (RUNFILES_MANIFEST) {
    Runfiles.readManifest(runfiles, RUNFILES_MANIFEST);
}
const getRunfile = name => runfiles.getPath(name);
commonjsGlobal.getRunfile = getRunfile;
const resolver = new ts.Resolver(TRACE, request => [
    request,
    `${request}.js`,
    request ? `${request}/index.js` : "index.js",
]);
ts.Resolver.readManifest(resolver, PACKAGES_MANIFEST, NODEJS_PACKAGES_RUNFILES == "true" ? getRunfile : (path) => path);
...
var index = /*@__PURE__*/getDefaultExportFromCjs(ts$1);
