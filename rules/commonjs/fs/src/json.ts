...
export namespace JsonFormat {
    ...
    export function map<K, V>(keyFormat: JsonFormat<K>, valueFormat: JsonFormat<V>): JsonFormat<Map<K, V>> {
        return new MapJsonFormat(keyFormat, valueFormat);
    }

    export function object<V>(format: { [K in keyof V]: JsonFormat<V[K]> }): JsonFormat<V> {
...
class MapJsonFormat<K, V> implements JsonFormat<Map<K, V>> {
    constructor(private readonly keyFormat: JsonFormat<K>, private readonly valueFormat: JsonFormat<V>) { }

    fromJson(json: any) {
        return new Map<K, V>(json.map(({ key, value }) => [this.keyFormat.fromJson(key), this.valueFormat.fromJson(value)]));
    }
