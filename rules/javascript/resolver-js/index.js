...
class Resolver {
  ...
  addPackage(id, package_) {
    ...
    for (const [name, file] of package_.runfileByName.entries()) {
      tslib_1
        .__classPrivateFieldGet(this, _pathToModule)
        .set(file, { name, package: package_ });
    }
  }
  /**
  ...
  _resolveDeps(deps, request, attempts) {
    for (const dep of deps) {
      const package_ = tslib_1
        .__classPrivateFieldGet(this, _packageById)
        .get(dep.id);
      if (!package_) {
  ...
  _resolveDep(request, id, package_, name, attempts) {
    ...
    attempts.push(id);
    for (const name of tslib_1
      .__classPrivateFieldGet(this, _variations)
      .call(this, requestPart)) {
...
(_trace = new WeakMap()),
  ...
  (_pathToModule = new WeakMap()),
  (_packageById = new WeakMap());
