...
if (config.module.rules) {
  config.module.rules = config.module.rules.map((rule) => {
    ...
    rule = { ...rule };
    if (rule.loader) {
      rule.loader = require.resolve(rule.loader);
    }
    if (rule.use instanceof Array) {
      rule.use = rule.use.map((use) => {
        ...
        use = { ...use };
        if (use.loader) {
          use.loader = require.resolve(use.loader);
        }
        return use;
      });
      ...
      rule.use = { ...rule.use };
      if (rule.use.loader) {
        rule.use.loader = require.resolve(rule.use.loader);
      }
    }
    return rule;
  });
...
config.resolveLoader = config.resolveLoader ? { ...config.resolveLoader } : {};
